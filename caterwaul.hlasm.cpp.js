...
// globals. Naturally the C++ generated by this backend is type-erased.
  ...
  caterwaul.tconfiguration('std seq continuation', 'hlasm.cpp', function () {
    ...
    where*[translate(globals)        = '// Global setup:\n#{prerequisites(globals)}\n// Main code:\nint main() {\n#{seq[sp[globals] *[global(_[0], _[1])]].join("\n")}}',
           ...
           statements()              = seq[~arguments *[_ instanceof Array ? statements.apply(this, slice(_)) : _] *[/^\/\//.test(_) ? '#{_}\n' : '#{_};\n']].join(''),
           ...
           global(name, v)           = v instanceof Array ? compile_constant(name, v) : statements.apply(this, slice(compile_bytecode(name, v))),
           ...
           compile_bytecode(name, s, vi_prime) =
             l*[result = seq[~[]], vi = vi_prime || {n: 0}, next() = vi.n < s.length && s.charAt(vi.n++), emit = result/mb/push, start = vi.n,
                compile_next() = next() /re.c[c === ']' || vi.n >= s.length ? emit('goto *c++', '// } from source ' + s.substring(start, vi.n - 1)) :
                                              c === '[' ? l*[gs = caterwaul.gensym(), xs = compile_bytecode(gs, s, vi)] in emit('*--d = &&#{gs}', compile_next() && [], xs) :
                         bytecode_translations[c] /re[_ ? emit(_()) : /\s/.test(c) || emit('// Undefined operation: "#{c}"')] /re[compile_next()]]] in
             emit('// Compilation of block #{name} {', '#{name}: ') /se[compile_next()]]});
