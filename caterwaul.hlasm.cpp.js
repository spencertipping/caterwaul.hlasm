...
// globals. Naturally the C++ generated by this backend is type-erased.
  ...
  caterwaul.tconfiguration('std seq continuation', 'hlasm.cpp', function () {
    ...
    where*[translate(globals)        = '// Global setup:\n#{prerequisites(globals)}\n// Main code:\nint main() {\n#{seq[sp[globals] *+global]}}',

           slice(x)                  = Array.prototype.slice.call(x),
           statements()              = seq[~arguments *[_ instanceof Array ? statements.apply(this, slice(_)) : _] *['  #{_};']].join('\n'),
           prerequisites(globals)    = statements('typedef long long e', 'typedef double f', 'static e data[1048576]', 'static e code[32768]',
                                                  'static e *d = data + 1048576', 'static e D[1048576]', 'static e *c = code + 32768', 'static e tmp'),
           ...
           compile_bytecode(name, vs) =
             l*[result = seq[~[]], vi = 0, next() = vs[vi++], emit = result/mb/push,
                compile_next() = next() /re.c[c === ']' || vi > vs.length ? seq[result + ['goto *c++']] :
                                              c === '['                   ? l[gs = caterwaul.gensym()] in emit('*--d = &&#{gs}', compile_bytecode(gs, seq[~vs.slice(vi + 1)])) :
                                                                            bytecode_translations[c] /re[_ && _()] /re[_ && emit(_), compile_next()]]] in compile_next()]});
