...
// globals. Naturally the C++ generated by this backend is type-erased.
  ...
  caterwaul.tconfiguration('std seq continuation', 'hlasm.cpp', function () {
    ...
    where*[translate(globals)        = l*[derived  = seq[sp[globals] *[compile(_[0], _[1])] /[caterwaul.util.merge(_, _0)]], prelude = prerequisites(derived) + ';\n\n',
                                          compiled = seq[~sk[derived].sort() -[['// Begin #{_}: #{derived[_].source.replace(/\n/g, "")}', derived[_].generated.join(';\n') + ';',
                                                                                '// End #{_}\n']]].join('\n')] in
                                       prelude + 'int main() {\n*--c = &&exit;\ngoto main;\nexit:\nreturn *d;\n\n#{compiled}\n}',

           gs                        = l[c = 0] in fn_['g#{++c}'],
           prerequisites(globals)    = ['typedef long long e', 'typedef double f', 'static e data[1048576]', 'static e code[32768]', 'static e dtmp[1048576]',
                                        'static e *d = data + 1048576', 'static e *D = dtmp', 'static e *c = code + 32768', 'static e tmp'].join(';\n'),
           ...
           bytecode_translations     = {} /se.ts[
             ...
             ts.i() = gs() /re['*--c = &&#{_}; goto **d++; #{_}:'], ts.j() = 'goto **d++', ts[']']() = 'goto **c++',
           ...
           compile(n, code) = l*[ns = seq[~[]], cs = seq[~[]], ss = seq[~[]], globals = {},
                                 encode(x) = bytecode_translations[x] /re['/* #{x} */ #{_ ? _() : ""}'],
                                 enter(n) = globals /se[cs.length && cs[cs.length - 1].push('/* [ */ *--d = &&#{n}'), ns.push(n), cs.push(['#{n}:']), ss.push([])],
